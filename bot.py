...
async def auto_post(context: ContextTypes.DEFAULT_TYPE):
    posts = [
        {"text": "Post 1", "image": "https://i.imgur.com/0Z1w3sD.png"},
        {"text": "Post 2", "image": "https://i.imgur.com/U1Cz4hG.png"},
    ]
    post = random.choice(posts)
    buttons = InlineKeyboardMarkup([[InlineKeyboardButton("Join Group", url=f"https://t.me/{GROUP}")]])
    for chat in CHANNELS + [GROUP]:
        try:
            await context.bot.send_photo(chat_id=chat, photo=post["image"], caption=post["text"], reply_markup=buttons)
