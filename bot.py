...
cursor.execute(
    """CREATE TABLE IF NOT EXISTS referrals (referrer_id INTEGER PRIMARY KEY, count INTEGER DEFAULT 0)"""
)
cursor.execute(
    """CREATE TABLE IF NOT EXISTS ads (ad_id INTEGER PRIMARY KEY AUTOINCREMENT, user_id INTEGER, text TEXT, link TEXT, amount REAL, status TEXT DEFAULT 'pending')"""
)
cursor.execute(
    """CREATE TABLE IF NOT EXISTS withdrawals (withdraw_id INTEGER PRIMARY KEY AUTOINCREMENT, user_id INTEGER, amount REAL, status TEXT DEFAULT 'pending')"""
)
conn.commit()
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if context.args:
        try:
            ...
            if ref != user_id:
                cursor.execute(
                    "INSERT OR IGNORE INTO referrals(referrer_id,count) VALUES(?,0)",
                    (ref,),
                )
                cursor.execute(
                    "UPDATE referrals SET count = count + 1 WHERE referrer_id=?", (ref,)
                )
                conn.commit()
...
async def approve(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    cursor.execute(
        "SELECT user_id, amount, status FROM ads WHERE ad_id=? AND status='pending'", (ad_id,)
    )
    ad = cursor.fetchone()
    if not ad:
        return await update.message.reply_text("❌ Ad not found or already approved")
...
async def approve_withdraw(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    cursor.execute(
        "SELECT user_id, amount, status FROM withdrawals WHERE withdraw_id=? AND status='pending'", (wid,)
    )
    req = cursor.fetchone()
    if not req:
...
def run_bot():
    ...
    # Start bot polling (this handles messages correctly)
    app.run_polling()
